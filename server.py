...
def normalize_site_settings(raw):
    ...
    booking_enabled = bool(raw.get("bookingEnabled", True))
...
def init_db() -> None:
    ...
    with get_db_connection() as conn:
        ...
        try:
            ...
            pass
        try:
            conn.execute("ALTER TABLE settings ADD COLUMN booking_enabled INTEGER NOT NULL DEFAULT 1")
        except sqlite3.OperationalError:
            # Column already exists in migrated databases.
            pass
...
def read_site_settings_from_db():
    with get_db_connection() as conn:
        ...
        return {
            ...
            "bookingEnabled": bool(row["booking_enabled"]),
    ...
    with get_db_connection() as conn:
        conn.execute(
            ...
            (
                ...
                1 if normalized["bookingEnabled"] else 0,
